...
class StartPage(QMainWindow):
    ...
    def load_recent_projects(self):
        ...
        try:
            ...
            # Create QMessageBox
            msg_box = QMessageBox()
            msg_box.setText("No recent projects file found. Starting fresh.")
            msg_box.setWindowTitle("Information")

            # Calculate center point of the screen
            center_point = QGuiApplication.primaryScreen().geometry().center()

            # Calculate the top-left corner position for the QMessageBox to center it
            top_left = center_point - msg_box.rect().center()

            # Set the position of the QMessageBox
            msg_box.move(top_left)
